...
- **Inject a custom geometry buffer into SSR** — `SSREffectDesc` accepts `ssr: { geometryBuffer: Texture | null }` to control where SSR applies (e.g. app-drawn puddles); `null`/unset falls back to the MRT normal buffer. The texture must use the G-buffer encoding `vec4(packNormalToVec2(viewNormal), mask, roughness)` — SSR skips pixels with `.z < 0.01`, and note the shader quirks: the ray tracer reads `.z` as its roughness base too, while `.w` drives the cone-tracing blur (default path) or multiplies `.z` for GGX jitter (cone tracing off). Texture *contents* updates need no API call; swapping the texture *object* goes through `handle.update({ ssr: { geometryBuffer } })`. Runnable reference: `example/pages/ssr-puddle/` (Navara repo). Effects built on the `postprocessing` library can alternatively declare `attributes: EffectAttribute.DEPTH` to receive composer depth in `mainImage` (see `SSREffect`). Real implementations to copy from (Navara repo): `navara_three_default_descs/src/effects/AerialPerspectiveEffectDesc.ts`, `CloudsEffectDesc.ts`, `ssr/SSREffect.ts`.
- **Terrain and the globe do NOT write the main MRT normal buffer** — their normals live in the separate globe copies (`ctx.getGlobeNormalTexture()`, `MRTPassEffectDesc.globeNormalBuffer`). In the main normal buffer, terrain pixels keep packed normal `(0, 0)` (which `unpackVec2ToNormal` decodes as `(0, 0, 1)` — a bogus camera-facing normal). Shaders that classify pixels by G-buffer normal must treat `dot(g.xy, g.xy) ≈ 0` as "no normal written", not as a valid normal (verified in `example/pages/ssr-puddle/`).
